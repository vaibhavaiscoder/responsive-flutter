...
class ResponsiveService {
  static const Size _designSize = Size(360, 819);
  ...
  static Size _switchableDesignSize() {
    return _mediaQueryData.orientation == Orientation.portrait ? _designSize : const Size(819, 360);
  }
  ...
  static double availableScreenHeight({double ratio = 1}) =>
      (_mediaQueryData.size.height - _mediaQueryData.viewPadding.vertical) * ratio;

  static double availableScreenWidth({double ratio = 1}) =>
      (_mediaQueryData.size.width - _mediaQueryData.viewPadding.horizontal) * ratio;
  ...
  static double scaleHeight() =>
      (_splitScreenMode ? max(availableScreenHeight(), 700) : availableScreenHeight()) / _switchableDesignSize().height;
